...
def test_validate_config_missing_nested_key():
    ...
    with open(os.path.join(test_path,"config_6_F.yaml"),"r") as file:
        is_valid,errors = validate_config(yaml.safe_load(file))
        assert is_valid == False
        assert errors == {"search_description": [{'simple': [{"heap_load_percent": ["required field"]}]}]}
...
def test_parse_config():
    ...
    assert expected_config.simulation_frequency_minutes == config.simulation_frequency_minutes
    assert expected_config.simulation_frequency_minutes == config.simulation_frequency_minutes
    assert expected_config.randomness_percentage == config.randomness_percentage
    assert expected_config.search_description == config.search_description
